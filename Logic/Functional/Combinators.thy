...
theory Combinators
  imports "../Proof/Intuitionistic/Minimal/Minimal_Logic"
begin
...
text {* The (polymorphic) typing for a combinator @{term X} is given by the 
        relation @{term "X \<^bold>\<Colon> \<phi>"}. *}

text {* Combinator types form an instance of minimal logic. *}

interpretation Combinator_Minimal_Logic: Minimal_Logic "\<lambda> \<phi>. \<exists> X. X \<^bold>\<Colon> \<phi>" "op \<^bold>\<Rightarrow>"
