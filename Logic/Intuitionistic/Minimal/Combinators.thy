...
text \<open>The fragment of the {@typ 'x SKCombinator} types without {@term Variable} terms
      can be given \<^emph>\<open>simple types\<close>:\<close>
...
text \<open>A useful lemma is the type of the \<^emph>\<open>identity\<close> combinator, designated by $\mathbf{I}$
...
text \<open> The minimal logic generated by combinator logic is \<^emph>\<open>free\<close> in the following sense:
...
text \<open> The fact that any valid type in combinator logic may be intrepreted in minimal logic is
        a form of the \<^emph>\<open>Curry-Howard correspondence\<close>. TODO: Cite \<close>
