...
datatype SKCombinator =
    Variable_Combinator Var ("\<^bold>\<langle>_\<^bold>\<rangle>" [100] 100)
  | S_Combinator ("S")
  | K_Combinator ("K")
  | Combinator_Application "SKCombinator" "SKCombinator"  (infixl "\<cdot>" 75)

text \<open> Note that in addition to \<^term>\<open>S\<close> and \<^term>\<open>K\<close> combinators, 
       \<^typ>\<open>SKCombinator\<close> provides terms for \<^emph>\<open>variables\<close>. This is helpful when 
       studying \<open>\<lambda>\<close>-abstraction embedding.\<close>

subsection \<open>Typing\<close>


text \<open>The fragment of the \<^typ>\<open>SKCombinator\<close> types without
      \<^term>\<open>Variable_Combinator\<close> terms can be given \<^emph>\<open>simple types\<close>:\<close>
...
inductive
Simply_Typed_SKCombinator
   :: "SKCombinator \<Rightarrow> 'a Simple_Type \<Rightarrow> bool" (infix "\<Colon>" 65)
where
...
primrec free_variables_in_SKCombinator :: "SKCombinator \<Rightarrow> Var set" ("free\<^sub>S\<^sub>K")
  where
...
primrec Turner_Abstraction 
  :: "Var \<Rightarrow> SKCombinator \<Rightarrow> SKCombinator" ("\<^bold>\<lambda> _. _" [90,90] 90)
  where
    abst_app: 
